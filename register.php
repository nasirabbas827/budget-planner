...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // If no errors, insert user into the database with status "pending"
    if (empty($username_err) && empty($password_err) && empty($email_err) && empty($phone_err) && empty($age_err)) {
        ...
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssis", $param_username, $param_password, $param_email, $param_phone, $param_age, $status);
        ...
        mysqli_close($conn);
        echo '<div class="alert alert-success" role="alert">User registration submitted. Pending admin approval.</div>';
    }
}
?>
